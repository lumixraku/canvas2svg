...
; (function () {
  ...
  ctx.prototype.__closestGroupOrSvgOnP = function (node) {
    node = node || this.__currentElement;
    if (node.nodeName === "ggg") {
      node = node.parentNode;
    }
    return this.__closestGroupOrSvg(node);
  };
  ...
  ctx.prototype.__parseFont = function () {
    var regex = /^\s*(?=(?:(?:[-a-z]+\s*){0,2}(italic|oblique))?)(?=(?:(?:[-a-z]+\s*){0,2}(small-caps))?)(?=(?:(?:[-a-z]+\s*){0,2}(bold(?:er)?|lighter|[1-9]00))?)(?:(?:normal|\1|\2|\3)\s*){0,3}((?:xx?-)?(?:small|large)|medium|smaller|larger|[.\d]+(?:\%|in|[cem]m|ex|p[ctx]))(?:\s*\/\s*(normal|[.\d]+(?:\%|in|[cem]m|ex|p[ctx])))?\s*([^]+?)\s*$/i;
  ...
  ctx.prototype.getTransform = function () {
    ...
    var matrixReg = transform.match(/matrix\((.*),(.*),(.*),(.*),(.*),(.*)\)/);
    if (matrixReg) {
      transformobj = {
        a: +matrixReg[1],
        b: +matrixReg[2],
        c: +matrixReg[3], //????? 如果是0 反而不显示了 // 会导致 clipcurr 变成 g 元素  正常的话都是 path // 用字符串和大于 2 的值都行 ???
        d: +matrixReg[4],
        e: +matrixReg[5],
        f: +matrixReg[6]
      }
    }
    var scaleReg = transform.match(/scale\((.*),(.*)\)/);
    if (scaleReg) {
      transformobj = {
        a: +matrixReg[1],
        b: 0,
    ...
    var translateReg = transform.match(/translate\((.*),(.*)\)/)
    if (translateReg) {
      transformobj = {
        a: 1,
        b: 0,
